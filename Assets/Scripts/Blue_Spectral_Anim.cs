...
public class Blue_Spectral_Anim : MonoBehaviour {
	...
	public int state = 0; //0 = wandering, 1 = attacking Player, 2 = attacking Spectral
	...
	void Update () {
		...
		//if (state == 2 && anim.GetCurrentAnimatorStateInfo(0).IsName("Charge_WindUp"))
		//{
			//target = Spectral.transform;
			//moveSpeed = 1;
		//}
	...
	void OnTriggerEnter(Collider other)
	{
		...
		else if (other.gameObject.tag == "Spectral") 
		{
			Vector3 relativePos = other.gameObject.transform.position - RayCastPoint.transform.position;
			RaycastHit hit;
			if(Physics.Raycast(RayCastPoint.transform.position, relativePos, out hit))
			{
				Debug.Log("Saw something");
				Debug.Log(hit.collider.tag);
				if(hit.collider.tag == "Spectral")
				{
					Debug.Log("Saw the Spectral");
					state = 2;
					//anim.SetTrigger("SeePlayer");
					attackTimer = 0.0f;
				}
